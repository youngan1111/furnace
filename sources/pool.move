module hello_blockchain::pool {
    ...
    public entry fun add_liquidity<X,Y>(account: &signer) {
        assert!(is_sorted<X, Y>(), 0);

        let account_addr = signer::address_of(account);

        let (min_qve_liq, min_usdf_liq) = router_v2::calc_optimal_coin_values<X, Y, Uncorrelated>(
            100000000,
            100000000,
            1,
            1
        ...
        let qve_liq = coin::withdraw<X>(account, min_qve_liq);
        let usdf_liq = coin::withdraw<Y>(account, min_usdf_liq);

        let (qve_remainder, usdf_remainder, lp) = router_v2::add_liquidity<X, Y, Uncorrelated>(
            qve_liq,
            min_qve_liq,
            usdf_liq,
            min_usdf_liq,
